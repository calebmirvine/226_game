...
def test_treasure_placement_case_3x3():
    random.seed(0)
    board = Board(3, '3')
    treasure_positions = [(r, c) for r in range(board.n) for c in range(board.n) if board.board[r][c] != '_']
...
def test_place_treasure():
    b = Board(2, "2")
    assert sum(cell != '_' for row in b.board for cell in row) == 3
...
def test_board_str_output():
    ...
    expected = '\n'.join([' '.join(['_' for _ in range(2)]) for _ in range(2)])
